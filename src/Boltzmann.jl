...
"""
function df_qse_condition(μ, T,rho, ap)
        sum_exp  = 0.0
        sum_expY = 0.0
        sum_exp_si = 0.0
        sum_expY_si = 0.0
        df11, df12, df21, df22, df13, df31, df33, df23, df32, tmp1, tmp2 = zeros(Float64, 11)
        ...
        for apᵢ in ap[144:end]
            pr_i = prefactor(apᵢ)(T, rho)
            exp_i_si = exp((μ[1] * (apᵢ.Z - 14) + μ[2] * ((apᵢ.A -apᵢ.Z) -14) + μ[3] - (apᵢ.Eb .- ap[144].Eb)) / (kmev * T))
            ...
            #println(μ[1] * apᵢ.Z + μ[2] * (apᵢ.A -apᵢ.Z) - apᵢ.Eb, μ[1] * (apᵢ.Z - 14) + μ[2] * ((apᵢ.A -apᵢ.Z) -14) + μ[3] - (apᵢ.Eb .- ap[144].Eb))
        end
...
"""
function qse_condition(μ, T::Real, ρ::Real, y::Real, x_qse::Real, ap)
        res = zeros(Real,2) # Real here for AtoDiff to work
        res_si = zeros(Real, 2)
        sol = zeros(Real, 3)
        for (in, apᵢ) in enumerate(ap[1:143])
            pr_i = prefactor(apᵢ)(T, ρ)
            exp_i = exp((μ[1] * apᵢ.Z + μ[2] * (apᵢ.A -apᵢ.Z) - apᵢ.Eb) / (kmev*T))
            factor_i = [1.0, apᵢ.Z / apᵢ.A]
            res .= (pr_i .* factor_i .* exp_i) .+ res
        end
        for (in, apᵢ) in enumerate(ap[144:end])
            pr_i = prefactor(apᵢ)(T, ρ)
            exp_i_si = exp((μ[1] * (apᵢ.Z - 14) + μ[2] * ((apᵢ.A -apᵢ.Z) -14) + μ[3] - (apᵢ.Eb .- ap[144].Eb)) / (kmev*T))
            factor_i = [1.0, apᵢ.Z / apᵢ.A]
